...
class UserPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        if not request.user.groups:
            raise NotAcceptable(detail='group has to be in the usr object')

        if '*' in USER_GROUPS.get(request.user.groups[0]):
            return True

        if not USER_GROUPS.get(request.user.groups[0]) or self.permission not in USER_GROUPS.get(request.user.groups[0]):
